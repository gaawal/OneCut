...
from app.schemas.movies import Voice
...
def get_all_azure_voices(filter_locals=None) -> list[dict]:
    if filter_locals is None:
        filter_locals = ["zh-CN","zh-HK", "zh-TW"]
    voice_dir = utils.voice_dir()
    voices_list_path = os.path.join(voice_dir, "azure-voices.txt")
    # 读取配置文件内容
    with open(voices_list_path, "r", encoding="utf-8") as file:
        voices_str = file.read().strip()
    voices = []
    name = ''
                ...
                if filter_locals:
                    for filter_local in filter_locals:
                        if name.lower().startswith(filter_local.lower()):
                            voices.append(Voice.from_name(name, gender).dict())
                else:
                    voices.append(Voice.from_name(name, gender).dict())
                name = ''
...
async def tts(text: str, voice_name: str, voice_file: str) -> [SubMaker, None]:
    ...
    return await azure_tts_v1(text, voice_name, voice_file)
...
def azure_tts_v2(text: str, voice_name: str, voice_file: str) -> [SubMaker, None]:
    ...
    for i in range(3):
        try:
            ...
            speech_key = movies_config.azure.get("speech_key", "")
            service_region = movies_config.azure.get("speech_region", "")
...
def _format_text(text: str) -> str:
    ...
    remove_chars = "[](){}"
    # 创建翻译表，将指定的字符替换为空格
    translation_table = str.maketrans(remove_chars, " " * len(remove_chars))
    # 应用翻译表，替换字符并去除首尾空白
